...
const COLS = [
    ...
    { key: 'angka_week', label: 'Week', sortable: true },
];
...
export default function WeekIndex({ weeks }) {
    const [tambahOpen, setTambahOpen] = useState(false);
    const [editOpen, setEditOpen]     = useState(false);
    const [editRow, setEditRow]       = useState(null);

    const addForm  = useForm({ angka_week: '' });
    const editForm = useForm({ angka_week: '' });

    const submitAdd = (e) => {
        e.preventDefault();
        addForm.post('/week/store', {
    ...
    const openEdit = (row) => {
        setEditRow(row);
        editForm.setData({ angka_week: row.angka_week });
        setEditOpen(true);
    };

    const submitEdit = (e) => {
        e.preventDefault();
        editForm.put(`/week/update/${editRow.id_week}`, {
            onSuccess: () => setEditOpen(false),
        });
    };

    const handleDelete = (row) => {
        if (!window.confirm(`Hapus week ${row.angka_week}?`)) return;
        router.delete(`/week/delete/${row.id_week}`);
    };
    ...
    const WeekInput = ({ form, formId }) => (
        <div className="mb-4">
            <label className="block text-sm font-medium text-slate-700 mb-1">Angka Week</label>
            <input type="number" min="1" value={form.data.angka_week} required
                onChange={(e) => form.setData('angka_week', e.target.value)}
            ...
            {form.errors.angka_week && <p className="text-xs text-red-600 mt-1">{form.errors.angka_week}</p>}
        </div>
    );

    return (
        <AppLayout title="MASTER WEEK" breadcrumb="Master / Week">
            <DataTable
                columns={COLS}
                data={weeks}
                actions={(row) => (
            ...
            <Modal open={tambahOpen} onClose={() => { setTambahOpen(false); addForm.reset(); }}
                ...
                <form id="add-form" onSubmit={submitAdd}>
                    <WeekInput form={addForm} />
                </form>
            </Modal>

            <Modal open={editOpen} onClose={() => setEditOpen(false)} title="Edit Week" size="sm"
