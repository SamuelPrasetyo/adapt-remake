...
const COLS = [
    ...
    { key: 'nama_nilai',label: 'Nama Nilai',  sortable: true },
];
...
export default function NilaiIndex({ nilais }) {
    const [tambahOpen, setTambahOpen] = useState(false);
    const [editOpen, setEditOpen]     = useState(false);
    const [editRow, setEditRow]       = useState(null);

    const addForm  = useForm({ nama_nilai: '' });
    const editForm = useForm({ nama_nilai: '' });

    const submitAdd = (e) => {
        e.preventDefault();
        addForm.post('/nilai/store', {
    ...
    const openEdit = (row) => {
        setEditRow(row);
        editForm.setData({ nama_nilai: row.nama_nilai });
        setEditOpen(true);
    };

    const submitEdit = (e) => {
        e.preventDefault();
        editForm.put(`/nilai/update/${editRow.id_nilai}`, {
            onSuccess: () => setEditOpen(false),
        });
    };

    const handleDelete = (row) => {
        if (!window.confirm(`Hapus nilai "${row.nama_nilai}"?`)) return;
        router.delete(`/nilai/delete/${row.id_nilai}`);
    };

    const modalFooter = (onCancel, formId, processing) => (
        <>
    ...
    return (
        <AppLayout title="MASTER NILAI" breadcrumb="Master / Nilai">
            <DataTable
                columns={COLS}
                data={nilais}
                actions={(row) => (
            ...
            <Modal open={tambahOpen} onClose={() => { setTambahOpen(false); addForm.reset(); }}
                title="Tambah Nilai" size="sm"
                footer={modalFooter(() => { setTambahOpen(false); addForm.reset(); }, 'add-form', addForm.processing)}>
                <form id="add-form" onSubmit={submitAdd}>
                    <div className="mb-4">
                        <label className="block text-sm font-medium text-slate-700 mb-1">Nama Nilai</label>
                        <input type="text" value={addForm.data.nama_nilai} required
                            onChange={(e) => addForm.setData('nama_nilai', e.target.value)}
                        ...
                        {addForm.errors.nama_nilai && <p className="text-xs text-red-600 mt-1">{addForm.errors.nama_nilai}</p>}
                    </div>
                </form>
            </Modal>

            <Modal open={editOpen} onClose={() => setEditOpen(false)} title="Edit Nilai" size="sm"
                footer={modalFooter(() => setEditOpen(false), 'edit-form', editForm.processing)}>
                <form id="edit-form" onSubmit={submitEdit}>
                    <div className="mb-4">
                        <label className="block text-sm font-medium text-slate-700 mb-1">Nama Nilai</label>
                        <input type="text" value={editForm.data.nama_nilai} required
                            onChange={(e) => editForm.setData('nama_nilai', e.target.value)}
                            className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500" />
                        {editForm.errors.nama_nilai && <p className="text-xs text-red-600 mt-1">{editForm.errors.nama_nilai}</p>}
                    </div>
                </form>
            </Modal>
        </AppLayout>
