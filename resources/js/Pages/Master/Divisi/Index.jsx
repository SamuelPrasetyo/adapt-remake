...
const COLS = [
    ...
    { key: 'nama', label: 'Nama Divisi', sortable: true },
];

function FormField({ label, value, onChange, error, required = true }) {
    return (
        <div className="mb-4">
...
export default function DivisiIndex({ divisis }) {
    ...
    const openEdit = (row) => {
        setEditRow(row);
        editForm.setData({ nama: row.nama });
        setEditOpen(true);
    };

    const submitEdit = (e) => {
        e.preventDefault();
        editForm.put(`/divisi/update/${editRow.id}`, {
            onSuccess: () => setEditOpen(false),
        });
    };

    const handleDelete = (row) => {
        if (!window.confirm(`Hapus divisi "${row.nama}"?`)) return;
        router.delete(`/divisi/delete/${row.id}`);
    };
    ...
    return (
        <AppLayout title="MASTER DIVISI" breadcrumb="Master / Divisi">
            <DataTable
                columns={COLS}
                data={divisis}
                actions={(row) => (
            ...
            >
                <form id="add-form" onSubmit={submitAdd}>
                    <FormField label="Nama Divisi" value={addForm.data.nama}
                        onChange={(e) => addForm.setData('nama', e.target.value)}
                        error={addForm.errors.nama} />
                </form>
            </Modal>
            ...
            >
                <form id="edit-form" onSubmit={submitEdit}>
                    <FormField label="Nama Divisi" value={editForm.data.nama}
