...
function TambahModal({ open, onClose, kaders, companys }) {
    ...
    // Password wajib diisi, min. 6 karakter, dan konfirmasi harus cocok.
    const pwdInvalid =
        !form.data.password ||
        form.data.password.length < 6 ||
        form.data.password !== form.data.password2;

    const submit = (e) => {
        e.preventDefault();
        if (!form.data.password) {
            setPwdError('Kata sandi wajib diisi');
            return;
        }
        if (form.data.password.length < 6) {
            setPwdError('Kata sandi minimal 6 karakter');
            return;
        }
            ...
            footer={
                <>
                    ...
                    <button
                        type="button"
                        onClick={submit}
                        disabled={form.processing || pwdInvalid}
                        className="px-4 py-2 text-sm font-medium text-white bg-blue-500 hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg transition shadow-sm shadow-blue-500/30"
                    >
            ...
            <div className="space-y-4">
                    ...
                    <>
                        <div>
                            ...
                            <input
                                ...
                                className="w-full rounded-lg border-slate-200 focus:border-blue-500 focus:ring-blue-500/30 text-sm"
                            />
                            {form.errors.nik_mentor && (
                                <p className="mt-1 text-xs text-red-600">{form.errors.nik_mentor}</p>
                            )}
                        </div>
                        <div>
                            ...
                            {form.errors.company_code && (
                                <p className="mt-1 text-xs text-red-600">{form.errors.company_code}</p>
                            )}
                        </div>
                        <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                        ...
                        {(pwdError || form.errors.password) && (
                            <p className="mt-1 text-xs text-red-600">{pwdError || form.errors.password}</p>
                        )}
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1.5">
