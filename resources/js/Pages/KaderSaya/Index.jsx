...
function MentorDropdown({ mentors, value, onChange, totalKaderInBatch }) {
    ...
    const current =
        value === "all" ? null : (mentors || []).find((m) => m.id === value);
    const totalKader =
        totalKaderInBatch ??
                ...
                <div className="absolute right-0 top-[calc(100%+6px)] w-72 z-30 bg-white rounded-xl shadow-xl ring-1 ring-slate-200 overflow-hidden">
                    ...
                    <div className="max-h-64 overflow-y-auto py-1">
                        ...
                        {filtered.map((m) => (
                            ...
                            >
                                <span
                                    className={`w-7 h-7 rounded-lg bg-gradient-to-br ${avatarColor(
                                        m.id
...
function BatchDropdown({ batches, value, onChange }) {
    ...
    const isActiveBatch = (b) => {
        ...
        const s = b.tanggal_selesai
            ? String(b.tanggal_selesai).slice(0, 10)
            : null;
        return !!m && !!s && m <= today && today <= s;
    };
    const batchLabel = (b) =>
        `Batch ${b.nama_batch}${b.tahun_batch ? ` (${b.tahun_batch})` : ""}`;
    const current =
        value === "all"
            ? null
            : list.find((b) => String(b.id_batch) === String(value));
    const label =
        value === "all"
            ? "Semua Batch"
            : current
                ...
                <div className="absolute right-0 top-[calc(100%+6px)] w-64 z-30 bg-white rounded-xl shadow-xl ring-1 ring-slate-200 overflow-hidden">
                    <div className="max-h-64 overflow-y-auto py-1">
                        {list.length === 0 ? (
                            ...
                            list.map((b) => {
                                const sel =
                                    String(value) === String(b.id_batch);
                                return (
                                    <button
                                        key={b.id_batch}
                                        type="button"
            ...
            <div className="p-5 flex-1">
                ...
                <div className="flex items-start gap-3 mb-4">
                    <KaderAvatar
                        ...
                        fallbackClass={`bg-gradient-to-br ${avatarColor(
                            kader.nik_kader || kader.nama_kader
                        )}`}
                    />
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-slate-500 truncate mt-0.5">
                            {kader.bu && (
                                <span className="inline-flex items-center px-1.5 py-0.5 rounded bg-blue-50 text-blue-600 text-[10px] font-bold ring-1 ring-blue-100 shrink-0">
                                    {kader.bu}
                                </span>
                            )}
                            <span className={kader.bu ? "ml-1.5" : ""}>
                                {kader.divisi_name || "—"}
                            </span>
                        </div>
                    </div>
                ...
                <div className="space-y-1">
                    ...
                    <div className="flex items-center justify-between">
                        <span className="text-[10px] text-slate-400">
                            {kader.total_moduls || 0} modul
                        </span>
                        {kader.batch_name && (
                            <span className="text-[10px] text-slate-400">
                                #Batch {kader.batch_name}
                                {kader.batch_year ? ` ${kader.batch_year}` : ""}
                            </span>
                        )}
                    </div>
                </div>
...
export default function KaderSayaIndex({
    kaders = [],
    mentors = [],
    selectedMentor,
    mentorFilter = "all",
